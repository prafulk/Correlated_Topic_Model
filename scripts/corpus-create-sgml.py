#!/usr/bin/env python
# Create a processed corpus from a collection of text files from a SGML collection
# Invocation: corpus-create <list-of-sgml-files>
...
doc_title_re = re.compile( r"<DOCNO>([^<]*)</DOCNO>" )
    ...
    with open(fname, "r") as f:
        ...
        for line in f:
            ...
            if doc_title_re.match( line ):
                title = doc_title_re.match( line ).groups()[0]
            if line == "<TEXT>":
...
def construct_dictionary( files ):
    ...
    for fname in files:
        for title, text in extract_text( fname ):
